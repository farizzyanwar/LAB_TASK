...
if($_SERVER ['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['FirstName'])) {
        $error[] = mysqli_real_escape_string($connect, trim($_POST['FirstName']));
    } else {
    ...
    if (empty($_POST['LastName'])) {
        $error[] = mysqli_real_escape_string($connect, trim($_POST['LastName']));
    } else {
...
if (mysqli_affected_rows($connect) == 1) {
    ...
    echo '<p class="error"> The user has not edited due to the system error. We apologize for any incovenience.</p>';
...
echo '<form action ="edit_doktor.php" method="post">
    ...
    <input id "FirstName" type ="text" name ="FirstName" size="30" maxlenght="30" value"'.$row[0].'"></p>

    <p><label class = "label" for="Lastname" > Last Name; </label>
    <input id "LastName" type ="text" name ="LastName" size="30" maxlenght="30" value"'.$row[1].'"></p>

    <p><label class = "label" for="Specializationr" >  Specialization; </label>
    <input id "Specialization" type ="text" name ="Specialization" size="30" maxlenght="30" value"'.$row[2].'"></p>

    <p><label class = "label" for="Password" > Password; </label>
    <input id "Password" type ="text" name ="Password" size="30" maxlenght="30" value"'.$row[3].'"></p>
